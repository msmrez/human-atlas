...
import {isSearchableConcept,matchesQuery} from './names.ts';
...
export function atlasTools(atlas:Atlas,inspect:(concept:Concept)=>void):Tool[]{return [
 {name:'find_anatomy',description:'Find anatomical structures by name, Persian name, or source atlas identifier in this atlas.',inputSchema:{type:'object',properties:{query:{type:'string',minLength:1}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:true},execute(input){const data=record(input);if(typeof data.query!=='string'||!data.query.trim())throw new Error('A nonempty query is required.');const q=data.query;return atlas.concepts.filter(c=>isSearchableConcept(c)&&matchesQuery(c.id,c.name,q)).slice(0,30).map(c=>({id:c.id,name:c.name,pieces:c.elements.length}));}},
