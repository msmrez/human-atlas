...
import {copy,hasOrganExplanation,structureExplanation} from './i18n';
import {useLocale,localeMeta} from './locale';
import {conceptLabel,partLabel,searchConcepts} from './names';
...
export default function Home(){
 ...
 const results=useMemo(()=>atlas?searchConcepts(atlas.concepts,query):[],[atlas,query]);
 ...
 return <main className="studio">
  ...
  <nav className="top-actions" aria-label={t.panelsAria}>
   <Button variant="ghost" className="locale-toggle" aria-label={t.languageAria} title={t.languageAria} onClick={()=>setLocale(nextLocale)}>{locale==='en'?'فا':'EN'}</Button>
  ...
  <section className={`layers-panel glass ${panel==='layers'?'mobile-open':''}`} aria-label={t.layersAria}>
   ...
   <div className="layer-presets">
    <Button variant="ghost" aria-pressed={activeSystems.every(x=>state.visible.includes(x.id))} onClick={()=>setState(s=>({...s,selected:[],isolate:false,visible:activeSystems.map(x=>x.id)}))}>{t.all}</Button>
